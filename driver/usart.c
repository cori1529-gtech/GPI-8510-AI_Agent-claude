...
void USART2_puts_rs422(char *str);
...
#define	MAX_USART2_BUFFER	10
u8	Usart2_Interrupt_flag;
u32	RxCounter2;
u8	RxBuffer2[MAX_USART2_BUFFER];
u8	cmd_2[MAX_USART2_BUFFER];//usart 2
...
//USARTx_putchar( USART1, buffer2);
		...
		if( buffer2 == '@' && RxCounter2 < 1) RxCounter2=0;
		
		RxBuffer2[RxCounter2++] = buffer2;
		
		if(RxCounter2 > 50) RxCounter2 = 0;
...
//		if( buffer2 == '@' && RxCounter2 < 1) RxCounter2=0;
		if( buffer2 == '@') RxCounter2 = 0;	// 2017.7.17
     	...
     	if(buffer2 == 0x0a)					//extern u8 Tmoney_Status_flag; // 티머니 카드 읽을때 1  아니면 0
	    {
			...
			RxCounter2 = 0;	// add 2017.7.17
		}
		
		if(RxCounter2 >= MAX_USART2_BUFFER) RxCounter2 = 0;
	}
}
*/
...
//USARTx_putchar( USART1, buffer2);
	 	...
	 	//@1-123456+0x0d+0x0a : total 11 byte 
			...
			//0X05 +ID(2BYTE)+CMD(3BYTE)+0X03 :TOTAL 7 BYTE	

 		if( buffer2 == 0X05 && RxCounter2 < 1) RxCounter2=0;
		
		RxBuffer2[RxCounter2++] = buffer2;
		
     	if(buffer2 == 0X03)					//extern u8 Tmoney_Status_flag; // 티머니 카드 읽을때 1  아니면 0
	    {
 			Usart2_Interrupt_flag = 1;		     
      cmd_2[0]=RxBuffer2[RxCounter2-6];	//0X05
 			cmd_2[1]=RxBuffer2[RxCounter2-5];	//ID
 			cmd_2[2]=RxBuffer2[RxCounter2-4];	//ID
 			cmd_2[3]=RxBuffer2[RxCounter2-3];	//COMMAND
 			cmd_2[4]=RxBuffer2[RxCounter2-2];	//COMMAND
 			cmd_2[5]=RxBuffer2[RxCounter2-1];	//COMMAND
...
//USART1_putchar(rxd);
	...
	///		else
			{
				//@# .... @# 때문에
				if(rxd == '#' || rxd == '$') { sms_receiving_flag = 1; RxCounter2_sms = 0; } // # : 기존 지테크 서버를 통해서 문자수신,  '$':LTE 자체 SMS 수신시
...
void USARTx_Initial()
{
	...
	USARTx_setting( USART2, 9600, 0);// for pc program , rs_422
...
//		init_usart1_buffer();
		...
		*/
		...
		GPIO_Init(UART_DEBUG_RX_PORT, &GPIO_InitStructure);
		// Configure USART1 Tx (PA.09) as alternate function push-pull 
		GPIO_InitStructure.GPIO_Pin = UART_DEBUG_TX_PIN;
		GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;
		GPIO_Init(UART_DEBUG_TX_PORT, &GPIO_InitStructure);
	}
...
//		init_usart1_buffer();
		...
		GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
		GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN_FLOATING;
		GPIO_Init(UART_RS485_RX_PORT, &GPIO_InitStructure);
		// Configure USART2 Tx (PA.2) as alternate function push-pull 
		GPIO_InitStructure.GPIO_Pin = UART_RS485_TX_PIN;
		GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;
		GPIO_Init(UART_RS485_TX_PORT, &GPIO_InitStructure);
	
		//485 TX ENABLE ACTIVE HIGH, 422 ALWAYS HIGH
		GPIO_InitStructure.GPIO_Pin =  UART_RS485_DE_PIN;	//485 TX ENABLE, NORMAL LOW
		GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
		GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
		GPIO_Init(UART_RS485_DE_PORT, &GPIO_InitStructure);
	}
...
//		init_usart1_buffer();
		...
		GPIO_InitStructure.GPIO_Pin = UART_MODEM_RX_PIN;
		GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
		GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN_FLOATING;
		GPIO_Init(UART_MODEM_RX_PORT, &GPIO_InitStructure);
		// Configure USART3 Tx (PB.10) as alternate function push-pull 
		GPIO_InitStructure.GPIO_Pin = UART_MODEM_TX_PIN;
		GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;
		GPIO_Init(UART_MODEM_TX_PORT, &GPIO_InitStructure);
...
//		init_usart1_buffer();
		...
		GPIO_InitStructure.GPIO_Pin = UART_ZIGBEE_RX_PIN;
		GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
		GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN_FLOATING;
		GPIO_Init(UART_ZIGBEE_RX_PORT, &GPIO_InitStructure);
		// Configure UART4 Tx (PC.10) as alternate function push-pull 
		GPIO_InitStructure.GPIO_Pin = UART_ZIGBEE_TX_PIN;
		GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;
		GPIO_Init(UART_ZIGBEE_TX_PORT, &GPIO_InitStructure);
	}
	else return;
...
{
	GPIO_SetBits(UART_RS485_DE_PORT, UART_RS485_DE_PIN);
	guRS422485TxEnableTimer = 6; //about 6 ms
}

void USART2TX_disable()
{
	GPIO_ResetBits(UART_RS485_DE_PORT, UART_RS485_DE_PIN);
}
void USART2_putchar(char ch)
{
//	int dly = 300;	//baudrate 115200
...
void USART3TX_enable()
{
	GPIO_SetBits(UART_MODEM_DE_PORT, UART_MODEM_DE_PIN);
	guRS422485TxEnableTimer = 6; //about 6 ms
}

void USART3TX_disable()
{
	GPIO_ResetBits(UART_MODEM_DE_PORT, UART_MODEM_DE_PIN);
}
...
void Initial_cmd_2_RxBuffer2(void)
{
	int iz;
	
	iz = MAX_USART2_BUFFER;
