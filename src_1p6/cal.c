...
void cal_mode()
{
  union { u32 dw; u16 u[2]; u8 c[4];} temp;
	...
	lcd70_show_string_mode_24x24(0, (Y_AXIS_SPACE_CAL+10)*1,"CALIBRATIION MODE", 16, 0);
...
//	lcd70_show_string_mode_24x24(0, Y_AXIS_SPACE_CAL*4,"    PRESS 1_KEY SELECT DIVISION", 16, 0);
	...
	v_division=Get_division();// 한눈의값 구하기 	  big font로 쓰기 
...
//	lcd70_show_string_mode_24x24(0, Y_AXIS_SPACE_CAL*6,"    PRESS 1_KEY SELECT CAPA", 16, 0);
	...
	lcd70_show_string_mode_24x24(300, Y_AXIS_SPACE_CAL*14,"(CAPACITY-PRESS KEY_1)", 16, 0);
...
////	lcd70_show_string_mode_24x24(0, Y_AXIS_SPACE_CAL*7, str_1, 16, 0);
     //RS422ON;
   	...
   	lcd43_show_big_num(X_START,Y_START+(Y_INCREMENT*2),3); //  "3:"
	lcd43_show_big_num(X_START+(X_INCREMENT*1),Y_START+(Y_INCREMENT*2),':');
...
//	Delay_ms(1000);
	...
	lcd43_show_big_num(X_START,Y_START+(Y_INCREMENT*3),4); //  "4:"
	lcd43_show_big_num(X_START+(X_INCREMENT*1),Y_START+(Y_INCREMENT*3),':');
	lcd70_show_string_mode_24x24(300, Y_AXIS_SPACE_CAL*25,"(LOADING WEIGHT-PRESS NUMBER KEY)", 16, 0);
...
//	lcd70_show_string_mode_24x24(0, Y_AXIS_SPACE_CAL*9, str_1, 16, 0);
   	...
   	lcd43_show_big_num(X_START,Y_START+(Y_INCREMENT*4),5); //  "5:"
	lcd43_show_big_num(X_START+(X_INCREMENT*1),Y_START+(Y_INCREMENT*4),':');
...
////	lcd70_show_string_mode_24x24(0, Y_AXIS_SPACE_CAL*12, str_1, 16, 0);
   	...
   	lcd43_show_big_num(X_START,Y_START+(Y_INCREMENT*5),6); //  "6:"
	lcd43_show_big_num(X_START+(X_INCREMENT*1),Y_START+(Y_INCREMENT*5),':');
	lcd70_show_string_mode_24x24(300, Y_AXIS_SPACE_CAL*36,"(CURRENT WEIGHT)", 16, 0);
...
//	lcd70_show_string_mode(300, 250,
	...
	lcd70_show_string_mode_24x24(0, Y_AXIS_SPACE_CAL*43, "CLALIBRATION FINISHED!!", 16, 0);
    lcd70_show_string_mode_24x24(0, Y_AXIS_SPACE_CAL*45, "REPOWER !!", 16, 0);
...
/////	lcd70_show_string_mode_24x24(0, Y_AXIS_SPACE_CAL*3,str, 16, 0);
   ...
   temp=test_division[which];

    lcd70_write_weight(X_START+(X_INCREMENT*2),Y_START,temp);	
...
//	u16 test_capacity[10]={5,10,15,20,25,30,35,40,45,50};// total 10 r
    u16 test_capacity[10]={5000,10000,15000,20000,25000,30000,35000,40000,45000,50000};// total 10 r

	u8 which=1; // means 140 kg 
	u8 exit_flag=0;	 
	u32 temp;
	...
	temp= test_capacity[which];
    lcd70_write_weight(X_START+(X_INCREMENT*2),Y_START+(Y_INCREMENT*1),temp);
	  ...
	  // if( Three_Weight_Ok)
	   {	
		 ...
		 raw_data=(zero_1+zero_2+zero_3+zero_4);
		  ...
		  lcd70_write_weight(X_START+(X_INCREMENT*2),Y_START+(Y_INCREMENT*5),lWeight);
...
//	lcd70_show_string_mode_24x24(0, Y_AXIS_SPACE_CAL*9, str_1, 16, 0);
	 ...
	 lcd70_write_weight(X_START+(X_INCREMENT*2),Y_START+(Y_INCREMENT*3),v_setting_weight);
