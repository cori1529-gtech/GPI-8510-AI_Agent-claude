...
volatile int  gxiAdcFlag;		// ad conversion end flag
volatile long gxlRawAdc;		// raw adc value
volatile long gxlSpanAdc;		// gxlRawAdc - v_zero
volatile long gxlFactoredAdc;	// gxlRawAdc * gnfFactor
volatile long gxlCountAdc;		// internal weight counter
volatile long gxlPreAdc;		// previous gxlFactoredAdc  : prev_adc1
volatile long gxlRawWeight;		// raw_span weight - not compensation
volatile long gxlWeight;		// net weight
volatile long gxlGrossWeight;	// gross weight
volatile long gxlWeight_2;		// net weight
volatile long gxlGrossWeight_2;	// gross weight
 ...
 signed long gplCompensationWeight2; 

 unsigned long gxwStatusSendInterval;	//상태정보 전송 시간(분)
unsigned long  gnlLcCompensation;	// 로드셀 죽었을 때 보상 ON/OFF
 
 
 long temp_weight; 
  
 signed long gnlDailyTotal;
 volatile signed long gnlPreDailyTotal[7];
 unsigned int gnuDay;
 int		  giToday;
...
signed long v_adc1_buf,raw_data,diff1,diff2,diff21,v_i_value;
...
volatile u8 One_Weight_Ok;
volatile u8 Two_Weight_Ok;
volatile u8 Three_Weight_Ok;
volatile u8 Four_Weight_Ok;
volatile u8 Five_Weight_Ok;
volatile u8 Six_Weight_Ok;
...
volatile unsigned char	Weight_flag; // 1 // 모듈로 부터 a/d 값 수신 ok

unsigned char	working_flag = 0;		/* working between start and stop */
unsigned char	working_flag_2 = 0;		/* working between start and stop   for silo2*/
