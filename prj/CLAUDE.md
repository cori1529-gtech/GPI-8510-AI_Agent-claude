...
- **같은 변수가 여러 헤더에 서로 다른 타입으로 선언된 경우가 있다** — 이 프로젝트에서는
  `set.h`와 `var.h`가 `gnlDailyTotal`/`gnlPreDailyTotal`을 각각 `unsigned long`/
  `signed long`으로 다르게 선언한다(실제 정의도 set.c/var.c에 이중으로 존재 — 링커가
  관용적으로 병합해 지금까지 안 드러난 잠재 버그). 새 파일에 이 두 헤더를 동시에 include
  하면 컴파일 에러가 난다. 한쪽만 include하거나 개별 `extern`으로 우회할 것.
