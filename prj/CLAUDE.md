...
### 10.3 driver 계층 핀 매크로 배선 현황 (2026-07 완료)

목표: "새 PCB = board_gpi8510.h 한 파일만 수정"을 실현하려면 각 driver의 raw 핀 리터럴
(`GPIO_Pin_5` 등)을 bsp 매크로로 바꿔야 한다. **원칙: 매크로 값이 원래 리터럴과 정확히
같은 경우에만 치환한다(값이 다르면 동작이 바뀜). 주석/죽은 코드는 건드리지 않는다.**

| 파일 | 상태 | 비고 |
|------|------|------|
| adc.c | ✅ 원래부터 완전 매크로 | raw 리터럴 0 |
| usart.c | ✅ 완료 | 남은 1곳은 `/* */` 주석 안의 죽은 485 DE 코드 → 방치 |
| led.c | ✅ 완료 | board에 `SILO1/2_LED_*`, `SILO1/2_ERR_LED_*`(GPIOF 7/9/8/10) 신설 |
| buzzer.c | ✅ 완료 | board에 `RELAY1/2/3/5_*`, `BATTERY_PWR_*` 신설. First~Fourth_Relay=OUT1~4 재사용 |
| key.c | ✅ 부분완료 | STATUS LED/Junction/Bat/Rtc는 매크로화. **단, key.c는 예전에 UTF-8로 깨진 파일(한글주석 U+FFFD 113개)이라 cp949 아님 — utf-8로 다루고 U+FFFD 개수 불변 검증** |
| lcd.c | ✅ 변경없음 | 실제 표시경로(GLCD_string→lcd_data_write)는 이미 `LCD_E/RW/DI` 매크로 사용 |

**남겨둔(건드리면 안 되는) raw 리터럴:**
- **key.c의 `First~Fiveth_digit_On()` (GPIOG Pin_10~14)**: 옛날 키매트릭스 스캔 방식(죽은 경로).
  이 핀들은 board에서 **LCD 제어선(CS1=11,CS2=12,E=13,RW=14)과 겹친다** — 실제 키 입력은
  `CHKEYIN()`(GPIOE) → `keypush_8510()` 경로다. 절대 SILO/LCD 매크로로 바꾸지 말 것.
- **lcd.c의 `lcdu8()` (GPIOG Pin_9/10/11)**: 텍스트 LCD(HD44780) 방식으로, 그래픽 LCD board
  매크로(Pin_13/14/15)와 **핀 값이 다르다**. `src/test.c`에서만 호출되는 옛날/테스트 경로라
  board 매크로로 치환하면 동작이 깨진다. 그대로 둔다.
- **usart.c 485 DE (`GPIOB Pin_12`)**: `/* */` 주석 처리된 죽은 코드.
