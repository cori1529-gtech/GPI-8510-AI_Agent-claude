...
// VER 2.6, 전송해야할  IP 가 2개일때   차례대로 보내기 위한 순서
...
void tcpip_send_20140806(char *ip, int port)//0303
{
	int	ret_val = 0;
	char temp[30];
  sprintf(temp,"%d.%d.%d.%d",(int)IpAdresss_1,(int)IpAdresss_2,(int)IpAdresss_3,(int)IpAdresss_4);
	...
	switch(giSendingType)
	{
		...
		// USART1_puts("********************giSendingType=0*****************");
			
		
		/*
			websms_connection();
			if(iSendStage == 300) // 초기상태로 돌아왔음
			{	
				giSendingType++;
				iSendStage = 0;
				//get_cdma_time();
		...
		//USART1_puts("giSendingType=1");
	 //USART1_puts("********************giSendingType=1*****************");
				
	//	tcpip_send_form_zigbee_data(MONITORING_SERVER_IP, MONITORING_SERVER_PORT+giGtechServerCurrentPortIndex);
  //  tcpip_send_form_zigbee_data(MONITORING_SERVER_IP, MONITORING_SERVER_PORT);
	//2021년 3월 3일 사용자 설정(ip , 포트)에 따른 전송	
     tcpip_send_form_zigbee_data(temp, IpPort);  // 사용자설정
 ...
 */
   // gpi_8510
	   ...
	   /*
			  ...
			  { 
          sprintf( sz, "M%07ld|M%03ld|%s|%s|%lx|%ld|%ld|%ld|%ld|%ld|%ld|%ld|%ld|%ld|%c|%ld|%ld|%ld|%ld|%ld|%ld|%ld|%ld|%ld|%c",
					 gnlSerialNo, gnlMyAddress, msg, VERSION, gnlTick, gxlWeight, gnlDailyTotal%100000,
					 (long)gnlPreDailyTotal[0]%100000, (long)gnlPreDailyTotal[1]%100000, (long)gnlPreDailyTotal[2]%100000, (long)gnlPreDailyTotal[3]%100000,
					 (long)gnlPreDailyTotal[4]%100000, (long)gnlPreDailyTotal[5]%100000, (long)gnlPreDailyTotal[6]%100000,
					 ((working_flag)?'W':'N')
				...
				{ 
          sprintf( sz, "M%07ld|M%03ld|%s|%s|%lx|%ld|%ld|%ld|%ld|%ld|%ld|%ld|%ld|%ld|%c|%ld|%ld|%ld|%ld|%ld|%ld|%ld|%ld|%ld|%c",
					 gnlSerialNo, gnlMyAddress, msg, VERSION, gnlTick, (gxlWeight+gxlWeight_2), (gnlDailyTotal+gnlDailyTotal2)%100000,
					 (long)(gnlPreDailyTotal[0]+gnlPreDailyTotal2[0])%100000, (long)(gnlPreDailyTotal[1]+gnlPreDailyTotal2[1])%100000, (long)(gnlPreDailyTotal[2]+gnlPreDailyTotal2[2])%100000, (long)(gnlPreDailyTotal[3]+gnlPreDailyTotal2[3])%100000,
       ...
       */
 //2021년 3월 3일   총누적배출량 및 장비 현재시간 함께 전송

Get1381(); //시간을 읽어오고  맨마지막에  측정시간(시간,분)을 보낸다.  
	 
	 //gxhTotalOutput=1234567;
  	 if(gnlSilo1Selection==1)
			  { 
					
					/*
          sprintf( sz, "M%07ld|M%03ld|%s|%s|%lx|%ld|%ld|%ld|%ld|%ld|%ld|%ld|%ld|%ld|%c|%ld|%02x-%02X",
					 gnlSerialNo, gnlMyAddress, msg, VERSION, gnlTick, gxlWeight, gnlDailyTotal%100000,
					 (long)gnlPreDailyTotal[0]%100000, (long)gnlPreDailyTotal[1]%100000, (long)gnlPreDailyTotal[2]%100000, (long)gnlPreDailyTotal[3]%100000,
					 (long)gnlPreDailyTotal[4]%100000, (long)gnlPreDailyTotal[5]%100000, (long)gnlPreDailyTotal[6]%100000,
					 ((working_flag)?'W':'N'),(long)gxhTotalOutput,(long)Gettimebuf[2],(long)Gettimebuf[1]);
          */			
        sprintf( sz, "M%07ld|M%03ld|%s|%s|%lx|%ld|%ld|%ld|%ld|%ld|%ld|%ld|%ld|%ld|%c|%ld|20%02x-%02X-%02X|%02x:%02x:%02x",
					 gnlSerialNo, gnlMyAddress, msg, VERSION, gnlTick, gxlWeight, gnlDailyTotal%100000,
					 (long)gnlPreDailyTotal[0]%100000, (long)gnlPreDailyTotal[1]%100000, (long)gnlPreDailyTotal[2]%100000, (long)gnlPreDailyTotal[3]%100000,
					 (long)gnlPreDailyTotal[4]%100000, (long)gnlPreDailyTotal[5]%100000, (long)gnlPreDailyTotal[6]%100000,
					 ((working_flag)?'W':'N'),(long)gxhTotalOutput,Gettimebuf[6],Gettimebuf[4],Gettimebuf[3],Gettimebuf[2],Gettimebuf[1],Gettimebuf[0]);					

					}

       else
				{ 
          sprintf( sz, "M%07ld|M%03ld|%s|%s|%lx|%ld|%ld|%ld|%ld|%ld|%ld|%ld|%ld|%ld|%c|%ld|%ld|%ld|%ld|%ld|%ld|%ld|%ld|%ld|%c|%ld|20%02x-%02X-%02X|%02x:%02x:%02x",
					 gnlSerialNo, gnlMyAddress, msg, VERSION, gnlTick, (gxlWeight+gxlWeight_2), (gnlDailyTotal+gnlDailyTotal2)%100000,
					 (long)(gnlPreDailyTotal[0]+gnlPreDailyTotal2[0])%100000, (long)(gnlPreDailyTotal[1]+gnlPreDailyTotal2[1])%100000, (long)(gnlPreDailyTotal[2]+gnlPreDailyTotal2[2])%100000, (long)(gnlPreDailyTotal[3]+gnlPreDailyTotal2[3])%100000,
					 (long)(gnlPreDailyTotal[4]+gnlPreDailyTotal2[4])%100000, (long)(gnlPreDailyTotal[5]+gnlPreDailyTotal2[5])%100000, (long)(gnlPreDailyTotal[6]+gnlPreDailyTotal2[6])%100000,
					 ((working_flag)?'W':'N')
					 ...
					 (long)gnlPreDailyTotal2[0]%100000, (long)gnlPreDailyTotal2[1]%100000, (long)gnlPreDailyTotal2[2]%100000, (long)gnlPreDailyTotal2[3]%100000,
					 (long)gnlPreDailyTotal2[4]%100000, (long)gnlPreDailyTotal2[5]%100000, (long)gnlPreDailyTotal2[6]%100000,
					 ((working_flag_2)?'W':'N'),(long)gxhTotalOutput,Gettimebuf[6],Gettimebuf[4],Gettimebuf[3],Gettimebuf[2],Gettimebuf[1],Gettimebuf[0]);
		...
		case SEND_SETVALUE:	//설정값 전송
			...
			*/
			sprintf( sz, "M%07ld|M%03ld|SET|%s|%lx|%lu|%lu|%lu|%lu|%ld|%lu|%lu|%lu|%lu|%lu|%lu|%ld|%ld",
						gnlSerialNo, gnlMyAddress, VERSION, gnlTick, gnlHenhouseNo, gnlUseLTE,
						gxhWeightBackup, gxhEndDelay, gplCompensationWeight,
						gnlSms_Send_Time_First,	gnlSms_Send_Time_Second, gnlSms_Send_Time_Third,
						Loadcell_Number, (long)Real_Weight_Factor, gnlDisplay, gxwStatusSendInterval, gnlLcCompensation
			);
			break;
		...
		default:
			...
			***/
			...
			sprintf( sz, "M%07ld|M%03ld|ST|%s|%lx|%lu|%d|%ld|%lu|%lu|%lu|%lu|%lu|%ld|%lu|%c%c%c%c%c%c",
						...
						((mon_One_Weight_Ok)?'G':'N'), ((mon_Two_Weight_Ok)?'G':'N'), ((mon_Three_Weight_Ok)?'G':'N'), ((mon_Four_Weight_Ok)?'G':'N'),
						((mon_Five_Weight_Ok)?'G':'N'), ((mon_Six_Weight_Ok)?'G':'N')
			);
			**/
			sprintf( sz, "M%07ld|M%03ld|ST|%s|%lx|%lu|%d|%ld|%lu|%lu|%lu|%lu|%lu|%ld|%lu|%c%c%c%c%c%c|%lu|",
						gnlSerialNo%10000000, gnlMyAddress%1000, VERSION, gnlTick, gnlHenhouseNo%100, gnuDay, mon_gxlGrossWeight%100000,
						Loadcell_Number%10, mon_One_Weight%1000000, mon_Two_Weight%1000000, mon_Three_Weight%1000000, mon_Four_Weight%1000000, mon_raw_adc, mon_Six_Weight%1000000,
						((mon_One_Weight_Ok)?'G':'N'), ((mon_Two_Weight_Ok)?'G':'N'), ((mon_Three_Weight_Ok)?'G':'N'), ((mon_Four_Weight_Ok)?'G':'N'),
						((mon_Five_Weight_Ok)?'G':'N'), ((mon_Six_Weight_Ok)?'G':'N'), mon_Five_Weight%1000000
			);
...
//		Send_status();
//		Send_setting();
/////////		Send_to_server(SEND_STATUS, "status"); 20170912
...
//USART1_puts(&RxCdmaBuffer[22]);
					Keco_card_info_flag = 0;	//cdma data 받아오을때 
					   if(RxCdmaCounter==44)// 페가수수
					 RTC_date_to_sec( &RxCdmaBuffer[22]);
				else// if(RxCdmaCounter==42)// 그리폰
					 RTC_date_to_sec( &RxCdmaBuffer[23]);
...
//USART1_puts("GetTime_res_ok");
			...
			*/		
						for(iz=0; iz<11; iz++)
					{
						if(RxCdmaCounter==39)// 페가수수  
						telno[iz] = RxCdmaBuffer[iz+20];  ////////// 페가수수
						else if(RxCdmaCounter==40) // 40, 그리폰
						  telno[iz] = RxCdmaBuffer[iz+21];    //////////그리폰
						
						if(('0'<=telno[iz]) && (telno[iz]<='9')) continue; //전화번호를 
					}
...
//USART1_puts("GetTime_res_ok");
				...
				}// of if if(RxCdmaCounter==41)//페가수수
				...
				{	
					...
					{
						ret_val = 6400;
						jangbi.cdma.rssi = (RxCdmaBuffer[24] - '0');
					}
					else if(26 == iz)
					{
						ret_val = 6400;
						jangbi.cdma.rssi = (RxCdmaBuffer[24] - '0')*10 + (RxCdmaBuffer[25] - '0');
					}
          ...
          /*				
				...
				}// ofif(check_response(RxCdmaBuffer, szResponse, RxCdmaCounter, strlen(szResponse)))
...
for(iz=0; iz<RxCdmaCounter; iz++) DEBUG_putchar(RxCdmaBuffer[iz]);
	...
	{	
		ret_val = 1;
       	USART1_puts("::$LGTTCPCL:0::"); //전송이 정상으로 끝났다.
...
void tcpip_send_form_zigbee_data(char *ip, int port)
{
	...
	char s[300];
	char ifIpSameFlag=0; // 사용자 설정 IP(위화살표+엔터키)와 지테크 서버 MONITORING_SERVER_IP	"112.217.212.250" , 8391
	                              // 같으면 1, 다르면 0

	
	if(IpAdresss_1==112 && IpAdresss_2==217 &&IpAdresss_3==212 &&IpAdresss_4==250 &&IpPort==8391) 
		  ifIpSameFlag=1;
     else
     ifIpSameFlag=0;			 
	...
	{
		case 0:  //0303
			/////// cdma 로 보낼데이타가 있는지 판단
	//	USART1_puts("***************** iSendStage=0****************");
...
///USART1_puts(str);
		...
		case 20://0303
			/////소켓이 close 되었는지 확인하고 안되어 있으면 소켓을 close 한다.
		  USART1_puts("***************** START OF SENDING ****************");
			SocketClose_LU202F();// TCP 소켓 CLOSE 여부 확인하고 안되어 있으면 소켓을 CLSOE 한다.
			iSendStage = 22;
			iStabeTwoWaitCount = 0;
			break;

		case 22: // 해당하는 IP, 포트의 소켓을 연다.

		  USART1_puts("***************** iSendStage=22****************");
      
		 
		 if(ifIpSameFlag==1)  // 사용자 설정 ip 와 지테크 ip 가 같으면 지테크 ip 한군데만 보내고
			 iSendStage = tcpip_send_case_20(MONITORING_SERVER_IP, MONITORING_SERVER_PORT, iStabeTwoWaitCount);
		 
		   else //// 사용자 설정 ip 와 지테크 ip 가 다르면  따로따로 보낸다.
			 {	 
		 
      if(Sending_Order==0)    // 사용자가 설정한(위화살표+엔터키) IP 및 포트로 전송한다.
			 iSendStage = tcpip_send_case_20(ip, port, iStabeTwoWaitCount);
	    else if(Sending_Order==1) // 원래 8510이 전송하는 곳으로 전송한다.	  
			iSendStage = tcpip_send_case_20(MONITORING_SERVER_IP, MONITORING_SERVER_PORT, iStabeTwoWaitCount);
		  }
	//	 	tcpip_send_form_zigbee_data(MONITORING_SERVER_IP, MONITORING_SERVER_PORT);
		  ...
		  //$LGTTCPOP:0 , 소켓OPEN  성공   ,iSendStage=40 
		  //$LGTTCPOP:4 , 소켓OPEN  FAIL   ,iSendStage=100
		  //$LGTTCPOP:0     
		
			break;
		
		case 40: // 소켓OPEN 이 성공되면  데이타를 CDMA를 통해 보낸다.
    //DEBUG_print("tcpip_send_form_zigbee_data() STAGE - 40 -- ");
		
		USART1_puts("***************** iSendStage=40****************");

     DEBUG_print("gszSendingGigbee::");
     DEBUG_puts(gszSendingGigbee);
			sprintf(s, "AT$LGTTCPWR=%d,%s", giSendingGigbeeLength, gszSendingGigbee); // 데이타를 보내는 명령어
			Stage_Four_Data_Sending_send_msg(s, strlen(s));  // 실제 CDMA 로 전송하는 함수
...
//DEBUG_puts("");		
			USART1_puts("***************** iSendStage=42****************");

		LGTTCPSTATE_LU202F(); //$LGTTCPSTATE:3-TCP 접속대기상태, $LGTTCPSTATE:4- TCP 접속 성공상태, $LGTTCPSTATE:5- TCP CLOSE 대기중인 상태, $LGTTCPSTATE:6: TCP CLOSE 성공상태
		
      switch(Stage_Four_Data_Sending_wait_response(iStage40Count++, 20))	//waiting server response  배출데이터를 전송후 리턴값을 기다림
			{
			 	case -1: //time out
					iSendStage = 100;
					iStage100Count = 0;
				case 1: //send ok 정상적으로 전송이 끝났으면 
...
//DEBUG_puts("");	
  ...
  // 이부분을 여길로 옮겼다.
		if(Sending_Order==0)
		USART1_puts("***************** FINISH  OF  USER IP, PORT SENDING !! ****************");
		if(Sending_Order==1)
		{	
		USART1_puts("***************** FINISH  OF  GTECH IP, PORT SENDING !! ****************");
		giGigbeeSendingFlag = 0;	//2014.8.25
		}
		
		Sending_Order++;
		if(Sending_Order==2)// 0,1 일때 2번 보냈음
		 Sending_Order=0;
...
*/
		...
		switch(Stage_Idle(iStage100Count++, SEND_INTERVAL))	//waiting send idle
			{
				case 1:
					iSendStage = 300;	//원상태로 만든다.  배출전송을 위한 초기상태로 돌아옴 ,iSendStage = 300
					break;
				default:
					break;
			}
			...
			// 이부분을 여길로 옮겼다.
   if(ifIpSameFlag==0)  // 사용자 설정 ip 와 지테크 ip 가 틀리면 따로따로 보내고
	 {		 
			if(Sending_Order==0)
		USART1_puts("***************** FINISH  OF  USER IP, PORT SENDING !! ****************");
		if(Sending_Order==1)
...
//					if(check_response(RxCardBuffer2, "R400", RxCardCounter2, 4)) //send ok test
		...
		if(iSendStage == 300)// 원상태로 오면 하나를 증가시킨다.
		{
			Sending_Order++;
		}	
			if(Sending_Order==2)// 0,1 일때 2번 보냈음
...
//DEBUG_puts("SocketClose_LU202F()----------------");
	...
	/*
	Query   AT$LGTTCPSTATE?
  Answer    $LGTTCPSTATE:<value>

0 : LTE 망 접속 안됨
2 : LTE 망 접속 성공
3 : TCP 접속 대기 중인 상태.
4 : TCP 접속 성공 상태.
5 : TCP Close 대기 중인 상태.
6 : TCP Close 성공 상태.
...
//DEBUG_puts("SocketClose_LU202F()------POINT-----");
	...
	do
	{
		...
		/*
		1) Device가 연결된 TCP Socket세션을 종료할 수 있도록 TCP Socket을 Close하는 명령
    이다.
     2) Socket 연결 해제는 해당 명령 외에, DTR 신호를 Toggle 시켜서도 수행 가능하다.
